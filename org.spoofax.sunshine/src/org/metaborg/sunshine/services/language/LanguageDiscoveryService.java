...
import static org.metaborg.sunshine.esvutil.ESVReader.observerFunction;
...
public class LanguageDiscoveryService {
	...
	public ALanguage languageFromEsv(IStrategoAppl document, Path basepath) {
		...
		for (IStrategoAppl action : builders) {
			String builderName = builderName(action);
			language.registerBuilder(builderName, builderTarget(action),
