...
// copied from package org.strategoxt.imp.runtime.services
...
public class InputTermBuilder {
	...
	public InputTermBuilder(HybridInterpreter runtime, Map<File, IStrategoTerm> resultingAsts) {
	...
	private IStrategoTerm mkSubListTarget(IStrategoTerm resultingAst, IStrategoList targetTerm, StrategoSubList node) {
		...
		if(firstChild == null || lastChild == null)
			return null;
		return new TermTreeFactory(Environment.INSTANCE().termFactory).createSublist(targetTerm,
	...
	public IStrategoTuple makeInputTerm(IStrategoTerm node, boolean includeSubNode,
		...
		File project = tryGetProjectPath(file);
		String path, projectPath;
		if (file != null && project != null) {
			// projectPath = project.toPortableString();
			// IPath fullPath = new Path(file.getAbsolutePath());
			// path = fullPath.removeFirstSegments(fullPath.matchingFirstSegments(project))
			// .setDevice("").toPortableString();
			// FIXME is this the same behaviour?
			projectPath = project.getAbsolutePath();
			path = file.getAbsolutePath().substring(projectPath.length());
	...
	private File tryGetProjectPath(File file) {
		if (file == null)
			return null;
		// FIXME verify if this is indeed proper behaviour
		// return EditorIOAgent.getProject(file).getLocation();
		return Environment.INSTANCE().projectDir;
	}
	 ...
	 */
	public IStrategoTuple makeATermInputTerm(IStrategoTerm node, boolean includeSubNode,
			File resource) {
		...
		String absolutePath = Environment.INSTANCE().projectDir.getAbsolutePath();
		String path = resource.getAbsolutePath().substring(absolutePath.length());
	...
	// IStrategoTerm node, boolean allowMultiChildParent) {
	// if (node == null)
	// return null;
	...
	// selectionNode = new
	// TermTreeFactory(Environment.getTermFactory()).createSublist((IStrategoList)
	// getParent(ancestor), ancestor, ancestor);
	// isMatch = isMatchOnConstructorOrSort(mappings, selectionNode);
	// }
	...
	// getParent(selectionNode) != null && !getParent(selectionNode).isList()) {
	// selectionNode = getParent(selectionNode);
	// isMatch = isMatchOnConstructorOrSort(mappings, selectionNode);
	// }
	// if (isMatch) {
	...
	// }
	//
	// public static ArrayList<NodeMapping<String>> createNodeMappings(IStrategoTerm[]
	// semanticNodes){
	...
	// public static boolean isMatchOnConstructorOrSort(ArrayList<NodeMapping<String>> mappings,
	// IStrategoTerm selectionNode) {
	// return selectionNode != null &&
	// NodeMapping.getFirstAttribute(mappings, tryGetName(selectionNode), getSort(selectionNode), 0)
	// != null;
	// }
