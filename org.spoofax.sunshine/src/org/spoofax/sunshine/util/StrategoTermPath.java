...
// copied from org.strategoxt.imp.runtime.stratego.StrategTermPath
// for porting to Sunshine
/**
...
public class StrategoTermPath {
	...
	private static final Logger LOG = LogManager.getLogger(StrategoTermPath.class);
	...
	public static IStrategoList toStrategoPath(List<Integer> pathInts) {
		...
		for (int i = 0; i < pathInts.size(); i++) {
			results.add(Environment.INSTANCE().termFactory.makeInt(pathInts.get(i)));
		}
	...
	/**
	 * Creates a term path given the AST Node of a parsed IStrategoTerm file. The resulting path
	 * relates to the actual AST, ignoring the 'appl' etc constructors of the IStrategoTerm syntax.
	 */
	public static IStrategoList createPathFromParsedIStrategoTerm(final IStrategoTerm node,
			Context context) {
		IStrategoTerm top = ParentAttachment.getRoot(node);
		final IStrategoTerm marker = context.getFactory().makeString(
		// ContentProposerSemantic.COMPLETION_TOKEN);
				// FIXME dirty hardcoded constant for porting to Sunshine
				"completion123");
	 ...
	 */
	public static IStrategoList getTermPathWithOrigin(Context context, IStrategoTerm ast,
		...
		if (isTermList(origin)) {
			...
			if (origin.getSubtermCount() > 0) {
				...
				if (subtermPath != null) {
					...
					// IStrategoTerm[] originPath = Arrays.copyOf(subtermPath.getAllSubterms(),
			...
			public IStrategoTerm invoke(Context context, IStrategoTerm current) {
				if (hasImploderOrigin(current)) {
					...
					if (currentImploderOrigin != null
							...
							&& ImploderAttachment.getLeftToken(currentImploderOrigin)
									.getStartOffset() == ImploderAttachment.getLeftToken(
									imploderOrigin1).getStartOffset()
							&& ImploderAttachment.getRightToken(currentImploderOrigin)
									.getEndOffset() == ImploderAttachment.getRightToken(
									imploderOrigin1).getEndOffset()) {
						...
						if (current.getTermType() == origin1.getTermType()) {
							if (current.getTermType() == IStrategoTerm.APPL) {
								...
								if (currentAppl.getName().equals(origin1Appl.getName())
					...
					// sets a term as 'nextBest' if one of the subterms of its origin-term is the
					// originChild
				...
				} else { // sets a term as 'nextBest' in case no origin term exists, but one of its
							// subterms is origin-related to the originChild
					if (current == origin1)
						return current;
					if (nextBest == null && originChild != null) {
		...
		if (perfectMatch != null) {
			...
			// Environment.logWarning("Could not determine term corresponding to " +
			// origin.toString() + " in resulting AST; using next best match " +
			// testOrigin.nextBest);
			testOrigin.origin1 = testOrigin.nextBest;
			return (IStrategoList) position_of_term_1_0.instance.invoke(context, ast, testOrigin);
		} else {
			// Environment.logWarning("Could not determine term corresponding to " +
			// origin.toString() + " in resulting AST");
			return null;
		}
	}

	public static IStrategoTerm getTermAtPath(Context context, IStrategoTerm term,
			IStrategoList path) {
	...
	public static IStrategoTerm findCommonAncestor(IStrategoTerm node1, IStrategoTerm node2) {
		...
		// Environment.logWarning("Could not find common ancestor for nodes: " + node1 + "," +
		// node2);
	 ...
	 */
	public static IStrategoTerm findCorrespondingSubtree(IStrategoTerm newAst,
			IStrategoTerm selection) {
		...
		for (int c = 0, size = selectionPath.size(); c < size; c++) {
			...
			if (i >= oldParent.getSubtermCount()) { // Shouldn't happen
				LOG.error("Unable to recover old selection AST in " + oldParent,
						new ArrayIndexOutOfBoundsException(i));
				// Environment.logException("Unable to recover old selection AST in " + oldParent,
				// new ArrayIndexOutOfBoundsException(i));
	...
	private static IStrategoTerm findCorrespondingSublist(IStrategoTerm newAst,
			StrategoSubList selection) {
	...
	private static boolean constructorEqual(IStrategoTerm first, IStrategoTerm second) {
		...
		return firstCons == null ? tryGetConstructor(second) == null : firstCons
				.equals(tryGetConstructor(second));
	}

	private static IStrategoTerm findCorrespondingSubtreeResult(IStrategoTerm oldAst,
			IStrategoTerm newAst, IStrategoTerm oldSubtree, IStrategoTerm newSubtree,
		...
		if (!newSubtree.equals(oldSubtree)) {
			...
			if (constructorEqual(newSubtree, oldSubtree)
					&& (containsMultipleCopies(newAst, newSubtree) || findSubtree(oldAst,
							newSubtree, true) == null)) {
	...
	/**
	 * Finds a unique, identical 'selection' subtree in 'newAst.' Returns null in case of multiple
	 * candidate subtrees.
	 */
	private static IStrategoTerm findIdenticalSubtree(IStrategoTerm oldAst, IStrategoTerm newAst,
			IStrategoTerm selection) {
	 ...
	 */
	private static IStrategoTerm findSubtree(IStrategoTerm ast, final IStrategoTerm selection,
